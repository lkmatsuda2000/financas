...
- **# projeção:** modelo no formato de planilha. Os meses fechados vêm da base (realizado); a partir do mês seguinte ao último com dados, tudo é calculado. As premissas são células do próprio P&L, na visão Mês (células azuis):
  ...
  A barra acima da tabela permite repetir um valor todo ano no mesmo mês ou em todos os meses seguintes. O valor a investir é um % do resultado do mês (receitas − gastos; mês negativo não aporta), dividido entre as classes pelo % digitado em cada uma (renda fixa, variável e outros), sem ajuste automático: se a soma ficar abaixo de 100%, o resto fica no caixa; acima, o excesso sai do caixa e a linha "Soma da divisão" fica vermelha. A diferença entre o resultado e os aportes vai para o caixa. As premissas são salvas automaticamente na tabela `premissas_projecao` (crie com `premissas_projecao.sql`, trocando o e-mail); horizonte, categorias de salário, classes das contas e valores de partida ficam em **Configurações**.
